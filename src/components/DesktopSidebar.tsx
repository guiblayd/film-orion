...
export function DesktopSidebar() {
  ...
  return (
    <aside className="hidden lg:flex lg:flex-col lg:gap-4 lg:sticky lg:top-6 lg:self-start">
      <div className="rounded-[28px] border border-zinc-800/60 bg-zinc-900/60 backdrop-blur-xl p-5">
        <div className="mb-6">
  ...
  return (
    <NavLink
      ...
      className={({ isActive }) => cn(
        ...
        isActive ? 'bg-zinc-100 text-zinc-950' : 'text-zinc-400 hover:bg-zinc-800/70 hover:text-zinc-100'
      )}
    >
      {({ isActive }) => (
        <>
          <div className="relative">
            <Icon size={18} className={isActive ? 'text-zinc-950' : undefined} />
            {badge > 0 && (
